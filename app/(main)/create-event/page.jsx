...
const eventSchema = z.object({
  ...
  city: z.string().min(1, "City is rewuired"),
...
const CreateEvent = () => {
  ...
  const cities = useMemo(() => {
    ...
    const district = bangladeshiDistricts.find(
      (d) => d.name === selectedDistrict
    );
    if (!district) return [];
    return City.getCitiesOfState("BD", district.isoCode);
  ...
  return (
    <div
      ...
      className="min-h-screen transition-colors duration-300 px-6 py-8 -mt-14 md:-mt:16 lg:rounded-md"
    >
        ...
        <div>
          ...
          {!hasPro && (
            <p className="text-sm text-muted-foreground">
              Free: {currentUser?.freeEventsCreated || 0}/1 created
            </p>
          )}
        </div>
        {/* Ai Event Creator */}
        ...
        >
          ...
          <div>
            <Input
              ...
              className="text-3xl sm:text-[15px] font-semibold bg-transparent border-none focus-visible:ring-0"
            />
            {errors.title && (
                  ...
                  >
                    ...
                    <SelectContent>
                      {bangladeshiDistricts.map((d) => (
                        <SelectItem key={d.isoCode} value={d.name}>
                          {d.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
